...
public class ClientSender {
	...
	private static void sendEcryptedAesKEY(OutputStream out, byte[] publicKey,byte[] aesKey) 
        ...
        System.out.println("Cipher Stream : " + cipherStream);
		cipherStream.close();
		tempByteStream.writeTo(out);
		String pid = new File("/proc/self").getCanonicalFile().getName();
		out.write(pid.getBytes("ASCII"));
	}
...
//        }
	...
	private static byte[] generateAesKey() throws NoSuchAlgorithmException {
        ...
        //System.out.println("AES key : "+secretAesKey);
        
        //copy aes key to file
        String fileName = "AES_key.txt";
        BufferedOutputStream bs = null;

        try {
            FileOutputStream fs = new FileOutputStream(new File(fileName));
            bs = new BufferedOutputStream(fs);
            bs.write(secretAesKey);
            bs.close();
            bs = null;
        } 
        catch (Exception e) 
	...
	public static void main(String[] args) {
		...
		try {
			...
			String pid = new File("/proc/self").getCanonicalFile().getName();
			System.out.println("Process ID of currently running Client process : " + pid);
